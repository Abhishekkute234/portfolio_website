  ...
  return (
    ...
    >
      ...
      <div className="w-full lg:w-1/2 flex flex-col p-6 sm:p-10 md:p-12 lg:p-16 relative z-10 justify-center min-h-[100dvh]">
        ...
        <motion.div variants={formVariants} initial="hidden" animate="show" className="w-full max-w-[460px] mx-auto bg-white/30 backdrop-blur-2xl border border-white/60 p-8 sm:p-12 md:p-14 rounded-[32px] shadow-[0_20px_60px_rgba(0,0,0,0.03)]">
          ...
          <motion.p variants={itemVariants} className="text-[15px] sm:text-base text-[#1A1A1A]/60 mb-10 font-light tracking-wide leading-relaxed">
            Join thousands of developers automating their creative processes.
          </motion.p>

          <form onSubmit={handleSubmit} className="flex flex-col gap-6 sm:gap-8">
            {error && <motion.div variants={itemVariants} className="bg-[#FFEBEB] text-[#D8000C] text-[13px] font-medium p-4 rounded-xl border border-red-100">{error}</motion.div>}
            ...
            <motion.div variants={itemVariants} className="space-y-2 group relative">
              ...
              <input type="text" required value={name} onChange={(e) => setName(e.target.value)} className="w-full bg-transparent border-b-[1.5px] border-[#1A1A1A]/10 hover:border-[#1A1A1A]/30 py-3 sm:py-4 focus:outline-none focus:border-[#1A1A1A] transition-all text-lg sm:text-xl tracking-wide placeholder:text-[#1A1A1A]/20" placeholder="John Doe" />
            </motion.div>
            ...
            <motion.div variants={itemVariants} className="space-y-2 group relative">
              ...
              <input type="email" required value={email} onChange={(e) => setEmail(e.target.value)} className="w-full bg-transparent border-b-[1.5px] border-[#1A1A1A]/10 hover:border-[#1A1A1A]/30 py-3 sm:py-4 focus:outline-none focus:border-[#1A1A1A] transition-all text-lg sm:text-xl tracking-wide placeholder:text-[#1A1A1A]/20" placeholder="hello@example.com" />
            </motion.div>

            <motion.div variants={itemVariants} className="space-y-2 group relative">
              <label className="text-[11px] font-bold tracking-[0.15em] uppercase text-[#1A1A1A]/40 group-focus-within:text-[#1A1A1A]/80 transition-colors block">Password</label>
              <input type="password" required value={password} onChange={(e) => setPassword(e.target.value)} className="w-full bg-transparent border-b-[1.5px] border-[#1A1A1A]/10 hover:border-[#1A1A1A]/30 py-3 sm:py-4 focus:outline-none focus:border-[#1A1A1A] transition-all text-lg sm:text-xl tracking-wide placeholder:text-[#1A1A1A]/20" placeholder="••••••••" />
            </motion.div>

            <motion.button variants={itemVariants} type="submit" disabled={loading} className="w-full mt-4 group relative overflow-hidden text-[#1A1A1A] py-[18px] sm:py-[20px] rounded-2xl text-[14px] sm:text-[15px] font-bold tracking-widest uppercase transition-all duration-500 disabled:opacity-70 flex justify-center items-center shadow-[0_8px_30px_rgba(218,238,71,0.3)] hover:shadow-[0_15px_40px_rgba(218,238,71,0.5)] hover:-translate-y-[2px]" style={{ background: 'linear-gradient(135deg, #DAEE47 0%, #c8dc35 100%)' }}>
              ...
              {loading ? <span className="w-5 h-5 border-2 border-[#1A1A1A]/30 border-t-[#1A1A1A] rounded-full animate-spin relative z-10" /> : <span className="relative z-10">Sign Up</span>}
            </motion.button>
          ...
          <motion.p variants={itemVariants} className="mt-10 text-center text-[13px] text-[#1A1A1A]/50 font-medium">
            ...
            <Link href="/login" className="text-[#1A1A1A] font-bold tracking-wide hover:text-[#1A1A1A]/70 border-b border-[#1A1A1A]/20 hover:border-[#1A1A1A]/50 pb-[1px] transition-all">
              Sign in
            </Link>
          </motion.p>
        </motion.div>
